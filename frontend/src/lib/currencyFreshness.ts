...
//
// Мерить это часами оказалось неверно. Источник публикует ДАТУ, а не момент,
// и вопрос «на сколько дней вперёд выписан курс» не зависит от того, в котором
// часу мы за ним пришли. Часовой порог смешивал две разные величины: календарное
// опережение публикации и время запуска обновления. Из-за этого один и тот же
// пятничный курс НБУ проходил в 17:26 (78,6 ч) и не проходил в 05:17 (90,7 ч).
//
// Поэтому опережение считается В КАЛЕНДАРНЫХ ДНЯХ, а предел задаётся по
// источнику: у каждого свой календарь публикации, и общее послабление выдавать
// всем без основания нельзя.
//
// НБУ публикует курс на СЛЕДУЮЩИЙ БАНКОВСКИЙ день, поэтому его опережение
// зависит от календаря выходных и праздников Украины:
//   вторник -> среда                              1 день
//   пятница -> понедельник                        3 дня
//   пятница перед праздничным понедельником -> вт 4 дня
//   пятница перед двумя праздничными днями -> ср  5 дней
// Шесть дней покрывают худший наблюдаемый случай с запасом в сутки и при этом
// отвергают дату на неделю и дальше: сломанный источник с датой через месяц или
// через год не пройдёт.
//
// ЕЦБ и НБМ публикуют курс на текущую дату, резервный источник — тоже. Им
// послабление не нужно, и они его не получают: двое суток оставлены только на
// разницу часовых поясов между запуском и публикацией.
export const PROVIDER_MAX_LEAD_DAYS: Record<string, number> = {
  nbu: 6,
};

export const DEFAULT_PROVIDER_MAX_LEAD_DAYS = 2;

export function providerMaxLeadDays(id: string): number {
  return PROVIDER_MAX_LEAD_DAYS[id] ?? DEFAULT_PROVIDER_MAX_LEAD_DAYS;
}

/**
 * Календарное опережение даты источника относительно текущей даты, в сутках.
 * Считается по датам в UTC, а не по разнице моментов: «на сколько дней вперёд
 * выписан курс» — свойство публикации, а не времени нашего запроса.
 */
export function providerLeadDays(date: string, now = new Date()): number {
...
export function assessProviderFreshness(
  ...
  // Опережение проверяется по календарным дням и по правилу своего источника.
  if (providerLeadDays(source.date, now) > providerMaxLeadDays(source.id)) {

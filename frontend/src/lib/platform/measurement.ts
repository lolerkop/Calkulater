...
/**
 * Оформление статистического показателя.
 *
 * Извлечено ПОСЛЕ трёх реальных потребителей: в описательной статистике,
 * средневзвешенном и z-оценке этот блок стоял побайтово одинаково. У статистики
 * разрядность выше, чем у размеров фигуры: стандартное отклонение 13,4907
 * отличимо от 13,491, и показатели одного набора обязаны быть согласованы между
 * собой — поэтому политика разрядов должна иметь один источник.
 *
 * Здесь только оформление: ни среднего, ни дисперсии, ни разбора списка.
 * Phase 13A измерила, что сами формулы и грамматики ввода у потребителей разные.
 */
...
/**
 * Синус угла, заданного в ГРАДУСАХ, с приведением машинного нуля к точному.
 *
 * Извлечено ПОСЛЕ трёх реальных потребителей: в параллелограмме, секторе круга и
 * моменте силы этот блок стоял побайтово одинаково. Общий здесь не столько сам
 * перевод, сколько порог: sin(π) в двоичной арифметике даёт 1,22·10⁻¹⁶, и хорда
 * полного круга, площадь при развёрнутом угле и момент вдоль рычага обязаны быть
 * РОВНО нулём. Порог должен быть один — разойдись он, один калькулятор показывал
 * бы нуль, а соседний остаток.
 *
 * Косинусный близнец живёт в калькуляторе работы силы с волны 3: у него один
 * потребитель, и ради одной строки released-калькулятор здесь не трогается.
 */
export const sinDegrees = (degrees: number): number => {

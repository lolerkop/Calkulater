...
export const compute: CalcFunction = (inputs) => {
  ...
  if (mode === 'voltage') {
    ...
    value = q / v;
    // Заряд и напряжение на одном конденсаторе всегда одного знака: обкладка,
    // заряженная положительно, и потенциал имеет выше. Разошедшиеся знаки дают
    // отрицательную ёмкость и отрицательную энергию поля — величины, которых
    // не существует, а не крайние значения существующих.
    if (!(value > 0)) return fail('Заряд и напряжение должны быть одного знака');
    farads = value;
    volts = v;
  } else {
    if (!(c > 0)) return fail('Ёмкость должна быть больше нуля');

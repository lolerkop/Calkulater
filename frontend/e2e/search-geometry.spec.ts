...
// Геометрия поля поиска в подборке.
//
// Та же ловушка, что и у поиска на главной, но своя разметка и свои размеры:
// лупа 16px при left-3, кнопка очистки 32px при right-2. Отступы утилитами
// (pl-10 pr-11) до поля не доходили — их перекрывало сокращённое `padding`
// у .field-input, и текст фактически начинался с 14px, заезжая под лупу,
// а длинный запрос уходил под кнопку. Отступы заданы правилами
// .catalog-search-input / .catalog-search-input-clearable, тест их закрепляет.
//
// Отдельный блок нужен потому, что до этого геометрия проверялась только на
// главной: поле подборки — другой компонент, и именно там дефект и жил.
const измеритьПодборку = (page: Page): Promise<Геометрия> => page.evaluate(() => {
  const input = document.querySelector('[data-testid="catalog-search"]') as HTMLInputElement;
  const icon = input.parentElement!.querySelector('svg')!;
  const clear = document.querySelector('[data-testid="catalog-search-clear"]');
  const style = getComputedStyle(input);
  const rect = input.getBoundingClientRect();
  const textStart = rect.left + parseFloat(style.borderLeftWidth) + parseFloat(style.paddingLeft);
  const textEnd = rect.right - parseFloat(style.borderRightWidth) - parseFloat(style.paddingRight);
...
// Подборка тоже гидрируется отложенно: печатать можно только после того,
// как с острова снят атрибут ssr, иначе React не увидит ввод.
async function ввестиВПодборке(page: Page, текст: string): Promise<void> {
  ...
  await input.fill(текст);
  await expect(page.getByTestId('catalog-search-clear')).toBeVisible({ timeout: 10000 });
}

const ПОДБОРКА: Array<[string, string, string]> = [
...
test.describe('поле поиска подборки не пускает иконки в текст', () => {
  for (const width of ШИРИНЫ) {
    test(`подборка на ${width}px: лупа не залезает в текстовую область`, async ({ page }) => {
      await page.setViewportSize({ width, height: 900 });
      for (const [path, loc] of ПОДБОРКА) {
        await page.goto(path);
        await page.getByTestId('catalog-search').waitFor();
        const g = await измеритьПодборку(page);
        expect(g.gapLeft, `${loc} @ ${width}px: зазор между лупой и текстом`).toBeGreaterThanOrEqual(МИН_ЗАЗОР);
        expect(g.overflow, `${loc} @ ${width}px: перелив документа`).toBe(0);
      }
    });

    test(`подборка на ${width}px: кнопка очистки не залезает в текстовую область`, async ({ page }) => {
      await page.setViewportSize({ width, height: 900 });
      for (const [path, loc, запрос] of ПОДБОРКА) {
        await page.goto(path);
        await ввестиВПодборке(page, запрос);
        const g = await измеритьПодборку(page);
        expect(g.gapRight, `${loc} @ ${width}px: зазор между текстом и кнопкой`).not.toBeNull();
        expect(g.gapRight!, `${loc} @ ${width}px: зазор между текстом и кнопкой`).toBeGreaterThanOrEqual(МИН_ЗАЗОР);
        // У подборки кнопка компактнее, чем на главной: контракт 32×32.
        expect(g.clearWidth, `${loc} @ ${width}px: размер кнопки очистки`).toBeGreaterThanOrEqual(32);
        expect(g.overflow, `${loc} @ ${width}px: перелив документа`).toBe(0);
      }
    });
  }

  test('подборка: широкий правый отступ появляется только вместе с кнопкой', async ({ page }) => {
    await page.setViewportSize({ width: 375, height: 800 });
    await page.goto('/ru/calculators/');
    const input = page.getByTestId('catalog-search');
    await input.waitFor();
    const пусто = await input.evaluate((el) => getComputedStyle(el).paddingRight);
    await ввестиВПодборке(page, 'ндс');
    const сЗапросом = await input.evaluate((el) => getComputedStyle(el).paddingRight);
    expect(parseFloat(сЗапросом), 'с запросом отступ должен быть больше').toBeGreaterThan(parseFloat(пусто));
    expect(parseFloat(пусто), 'на пустом поле лишнего провала быть не должно').toBeLessThan(24);
  });

  test('подборка: очень длинный запрос не заезжает под иконки', async ({ page }) => {
    const длинный = 'калькулятор сложного процента с ежемесячной капитализацией и регулярным пополнением вклада на длительный срок';
    for (const width of [320, 375, 1440]) {
      await page.setViewportSize({ width, height: 800 });
      await page.goto('/ru/calculators/');
      await ввестиВПодборке(page, длинный);

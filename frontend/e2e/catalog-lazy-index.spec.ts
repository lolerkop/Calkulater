...
test.describe('ленивый индекс каталога', () => {
  ...
  test('категория и сортировка забирают индекс ровно один раз и работают глобально', async ({ page }) => {
    ...
    // Запрос индекса и ПРИМЕНЕНИЕ индекса — разные события, и между ними
    // страница показывает выпущенный запасной путь: серверный срез, суженный
    // по категории. У «Финансов» это 29 карточек первой страницы против 47
    // членов раздела во всём каталоге. Считать в этом окне значит поймать
    // промежуточное число и потом сравнивать с ним устоявшееся.
    //
    // Признак применения — появление КЛИЕНТСКОЙ сетки: до него её нет в DOM
    // вовсе, после — серверная скрывается. Это тот же урок, что и с
    // `data-catalog-ready`: ждать надо клиентский признак, а не сетевой.
    await expect(page.locator('[data-catalog-global-grid]')).toBeAttached({ timeout: 15000 });

...
test.describe('поиск сохраняет ввод, сделанный до гидратации', () => {
  for (const [route, loc, запрос] of ХОЛСТ) {
    test(`${loc}: обычный запрос переживает гидратацию`, async ({ page }) => {
      ...
      // Индекс поиска приезжает отдельным файлом и только по обращению к поиску,
      // поэтому список появляется на такт позже гидратации. Ждём именно его:
      // проверяемое свойство — что выдача соответствует набранному запросу,
      // а не что она успевает к первому кадру.
  ...
  test('запрос без совпадений тоже восстанавливается', async ({ page }) => {
    ...
    отпустить();
    await дождатьсяГидратации(page);
    // «Ничего не найдено» показывается только после того, как индекс приехал:
    // до этого искать нечем, и заявлять об отсутствии совпадений было бы враньём.
    await page.getByTestId('search-empty').waitFor({ timeout: 10000 });
    const s = await состояние(page);
